...
export class LandingService {
  ...
  async getStatistics(): Promise<LandingStatistics> {
    try {
      ...
      const response = await apiClient.get('/landing/statistics');
  ...
  async getRecentActivities(limit: number = 10): Promise<Activity[]> {
    try {
      const response = await apiClient.get(`/landing/activities/recent?limit=${limit}`);
      return response.data;
    } catch (error) {
  ...
  async subscribeNewsletter(email: string, source: string = 'landing_page'): Promise<NewsletterResponse> {
    try {
      const response = await apiClient.post('/landing/newsletter', {
        email,
        source
      });
  ...
  async calculateROI(params: ROICalculationParams): Promise<ROIResult> {
    try {
      const response = await apiClient.post('/landing/calculate-roi', params);
      return response.data;
    } catch (error) {
